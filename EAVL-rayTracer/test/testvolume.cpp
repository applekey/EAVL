...
void SetUpAlpha(int tfSize) //eavlTransferFunction* myTransfer)
{
    //opac1
    cerr<<"HELLLLLLO\n";
    float pos; 
    for(int i=0; i<10; i++)
    {
        pos = i;
        cerr<<"Position "<<pos<<"\n";
	//myTransfer.AddAlphaControlPoint(pos,opac1[1]/255);
    }
...
{   
    ...
    {  
       ...
       // center = myMatrix->CreateTranslate(center);
        ...
        //myTransfer.AddAlphaControlPoint(0.5,0.5);
    ...
    for(int i=0; i<256; i++)
    {
        ...
        //for(int i=0; i<numCells; i++)
         // cerr<<" Num of Samples in Cell "<<i<<" is "<<samplesPerCell->GetValue(i)<<"\n";
